...
for day in day_list[-1:]:
    ...
    if 1:
        ...
        url = 'https://xueqiu.com/service/screener/screen?category=CN&exchange=sh_sz&areacode=&indcode=&order_by=symbol&order=desc&page=1&size=500&only_count=0&current=&pct=1_6&chgpct=0_10&pettm=0_13064.09&amount=50000000_7528546730.72&volume_ratio=1.2_5&pct5=1_20&pct20=-20_20&tr=0_10&pct10=1_20&npay.20230331=0_19080.47&_=1684488600966'
        res = requests.get(url=url, headers=headers).json().get("data").get("list")
        a_code =[]
        for i in  res:
            #print(i.get("f12")[:2])
            if i.get("symbol")[2:4] in ["60", "00"] and "ST" not in i.get("name"):
                #print(i.get("f14"))
                
                code = i.get("symbol")
                name = i.get("name")
                zhang_fu = i.get("pct")
                cheng_jiao_e = i.get("amount")/100000000
                dang_qian_jia = i.get("current")
                if 1:
                    ...
                    if day_df is not None:
                        ...
                        for x in ma_data_list:
                            if (max_ma * 0.975) <= x:
                                len_list.append(x)
                        #if "002134" in code:
                        #    print(dang_qian_jia, max_ma, ma5_distance, len(len_list))
                        if dang_qian_jia > max_ma and ma5_distance < 1 and len(len_list) >=(len(ma_data_list) -3):
                            #print(name + "--------------")
                            a_code.append(name)
