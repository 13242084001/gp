    ...
    if 1:
        ...
        for i in  data:
            ...
            if i.get("f12")[:2] in ["60", "00"] and "ST" not in i.get("f14") and "-" != i.get("f6"):
                ...
                """
                if 1 < zhang_fu < 6 and cheng_jiao_e > 1 and dang_qian_jia > kai_pan_jia and shang_ying_xian < 0.05 and huan_shou < 10 and liang_bi > 1:
                    ...
                    if day_df is not None:
                        ...
                        year_url = "https://stock.xueqiu.com/v5/stock/chart/kline.json?symbol={0}&begin={1}&period=year&type=before&count=-1&indicator=kline".format(code, timestamp)
                        month_url = "https://stock.xueqiu.com/v5/stock/chart/kline.json?symbol={0}&begin={1}&period=month&type=before&count=-300&indicator=kline".format(code, timestamp)
                        week_df = get_data_df(week_url)
                        month_df = get_data_df(month_url)
                        year_df = get_data_df(year_url)
                        ...
                        for index in range(-20,-1):
                            ...
                            shiti_high_20_list.append(tmp_high)
                        ...
                        for index in range(-10,-1):
                            data_dict = day_df.iloc[index].to_dict()
                            tmp_low = data_dict.get("low")
                            shiti_low_10_list.append(tmp_low)
                        
                        #ya_li_1 = (yes_close - max(shiti_high_11_list))/max(shiti_high_11_list) * 100
                        
                        year_high = year_df.iloc[-1].to_dict().get("high")
                        liang_max_5days = 0
                        ...
                        yes_ma40 = yesterday_data.get('ma40')
                        ...
                        atan40 = math.atan((ma40/yes_ma40 - 1)*100) * 180 / 3.1416
                        atan60 = math.atan((ma60/yes_ma60 - 1)*100) * 180 / 3.1416
                        atan_len = 0
                        for atan in [atan5, atan10, atan20, atan40, atan60]:
                            if atan > 10:
                                atan_len += 1
                        #print(atan5, atan10, atan20,atan40,atan60)
                        ...
                        for x in ma_data_list:
                            if (max_ma * 0.98) <= x:
                                len_list.append(x)
                        month_df = calc_zb(month_df)
                        month_data = month_df.iloc[-1].to_dict()
                        month_macd = month_data.get("bar")
                        #if "600020" in code:
                        ...
                        if dang_qian_jia > max(ma5, ma10, ma20, ma40, ma60, ma120, ma250) and ma20 > ma40 and ma60 > ma120 and (max(ma5, ma10, ma20)/min(ma5, ma10, ma20) - 1) < 0.06 and min(atan5, atan10, atan20) > 0 and dang_qian_jia/max(shiti_high_20_list) > 0.9 and (dang_qian_jia/min(shiti_low_10_list) -1) < 0.15 and dang_qian_jia/year_high > 0.8 and month_macd > 0:
                            print(name)
                            """
                            print(atan5, atan10, atan20,atan40,atan60)
                            print(code, dang_qian_jia, max_ma, ya_li, ya_li_1, fang_liang, today_liang, liang_max_5days, week_max_ma, ma5_distance, day_mavol5, day_mavol10, ma5, ma10, ma20,ma30,ma60, len(len_list), atan_len)
                            ...
                            if (atan_len >=3 and len(len_list) >= 5) or (jun_xian_distance < 3 and atan_len >=4 and len(len_list) >=4) or (atan_len >=5 and ma5 > ma10 > ma20 > ma40 > ma60 > ma250 and jun_xian_distance < 8) and 1 > day_macd > 0:
